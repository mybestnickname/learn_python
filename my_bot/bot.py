...
def planet_handler(bot, update):
    # отображаем данные о планете если не введена дата, то за сегодня
    # если введена, то за дату
    # /planet [Имя планеты] - сегодня
    # /planet [Имя планеты] [дата] - нахождение за дату
    # /planet - выведем список всех планет в которые может ephem
    ...
    bot_text = 'Планета {} находится в созвездии {}. Расстояние до земли: {} км. Дата: {}'.format(planet_name, ru_conts_name,
...
def solar_system_handler(bot, update):
    """
    Тут отрисовываются 9(8) планет нашей системы(кнопками), на каждую из которых можно
    кликнуть и посмотреть инфу о ней :)
    """
    pass
...
def constellations_translator(const_name):
    """
    Возвращает русское название или тоже самое, если нет в словаре
    """
    pass
...
def main():
    ...
    updt.start_polling(poll_interval=3)
    updt.idle()
