...
def start_handler(bot, update):
    ...
    bot_text = ('Привет {}. Для отображения списка команд /help .'.format(username))
...
def planet_handler(bot, update):
    ...
    full_constell = '{}({})'.format(ephem.constellation(planet_info)[1], ru_conts_name)
...
def main():
    ...
    updt.dispatcher.add_handler(MessageHandler(Filters.command, strange_command_handler))
    updt.start_polling()
    updt.idle()
